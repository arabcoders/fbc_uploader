...
async def test_download_blocked_for_disabled_token():
    """Test that downloads are blocked when token is disabled (even with admin key checks)."""
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        # Download should be blocked even with admin trying without explicit auth
        # Since public downloads are disabled by default, we need admin key
        # But even with admin key, if token is disabled, it should check that
        download_url = app.url_path_for("download_file", download_token=download_token, upload_id=upload_id)

        # Without admin auth, should get 401 (public downloads disabled)
...
async def test_download_blocked_for_expired_token():
    """Test that downloads are blocked when token is expired."""
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        # Create token and upload a file BEFORE it expires
        token_data = await create_token(client, max_uploads=1)
        upload_token = token_data["token"]
        download_token = token_data["download_token"]
        ...
        # Now update token to expire in the past
...
async def test_get_file_info_blocked_for_disabled_token():
    """Test that file info is blocked when token is disabled."""
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        # Create token and upload a file
        token_data = await create_token(client, max_uploads=1)
